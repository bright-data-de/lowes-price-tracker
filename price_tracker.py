"""
Lowe's Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Lowe's.

Docs  : https://docs.brightdata.de/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.de/products/insights/price-tracker/lowes
"""
...
if not API_TOKEN:
    raise EnvironmentError(
        ...
        "1. Get your token at: https://docs.brightdata.de/general/account/account-settings#api-token\n"
        "2. Add it to your .env file: BRIGHTDATA_API_TOKEN=<your_token>"
    )

# Find your Web Scraper ID at: https://brightdata.de/cp/scrapers
# Search for "Lowe's" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://brightdata.de/cp/scrapers\n"
        "2. Search for 'Lowe's' and copy the Web Scraper ID.\n"
        "3. Add it to your .env file: BRIGHTDATA_DATASET_ID=<your_scraper_id>"
    )

_BASE = "https://api.brightdata.de/datasets/v3"
_HEADERS = {
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Lowe's product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Lowe's products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Lowe's category page.

    Example:
        results = discover_by_category("https://lowes.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.lowes.com/pd/sample-product/1000123456",
        # Add more Lowe's product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "lowes_prices.json"
    ...
    # cat_results = discover_by_category("https://lowes.com/category/sample", limit=50)
